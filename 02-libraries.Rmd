...
RStudio Server is running at port 8787 (141.35.158.107:8787).
SSL certificate is coming soon (so we can have a https connection).

## Shiny Server

Is not yet usable because Shiny Server requires an R interpreter being available on a user basis when starting an application. 
This would mean that we need to load R in `~/.bashrc`. 
However, this would conflict with RStudio Server then.
