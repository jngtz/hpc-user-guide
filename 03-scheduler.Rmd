...
In contrast to running a "direct" R job from the command line on a server without a scheduler, jobs are submitted to the scheduler which then takes care of the execution while accouting for other running processes.

In R, this is done via the R package [rzmq](https://github.com/ropensci/rzmq) and [clustermq](https://github.com/mschubert/clustermq).
...
- You are forced to "functionize" your code, putting in into smaller pieces and diverge from long R scripts
